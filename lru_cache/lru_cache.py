...
class LRUCache:
  ...
  - max number of nodes if can hold
  ...
  - We want to find the least-recently-used entry - and we want to delete it
    ...
    - Array: pop()
    -Array: if we add to back, remove from the front: unshift()
  ...
  - We want many lookups by indentity
  ...
  Case 3: Additionally, in the case that the key already exists 
  in the cache, we simply want to overwrite the old value 
  associated with the key withthe newly-specified value. 
  """
